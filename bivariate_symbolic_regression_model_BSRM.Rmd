...
\begin{eqnarray}
\pmb \eta_{1} =
g_{1}(\pmb \mu_{1})=  \mathbf
X_{1}\pmb \beta_{1}}\,\,\,\mbox{and}\,\,\,
\pmb \eta_{2} =
g_{2}(\pmb \mu_{2})=\mathbf
X_{2}\pmb \beta_{2},
(\#eq:systematic)
\end{eqnarray}
where $\mathbf X_{1}$ and $\mathbf X_{2}$ are known model matrices
formed by the observed values of the variables $X_{1j}$ and $X_{2j}$, 
respectively, \boldmath$\beta$\unboldmath$_{1}$ and \boldmath$\beta$\unboldmath$_{2}$
are vectors of parameters to be estimated,
\boldmath$\eta$\unboldmath$_{1}$ and \boldmath$\eta$\unboldmath$_{2}$ are
the linear predictors, \boldmath$\mu$\unboldmath$_{1}$ and \boldmath$\mu$\unboldmath$_{2}$ 
are the mean of the res\-pon\-se variables $Y_{1}$ and $Y_{2}$, respectively, with
\mbox{\boldmath$\eta$\unboldmath$_{l}$}=$(\eta_{l_{1}},\ldots,
\eta_{l_{n}})^\top,$\mbox{\boldmath$\mu$\unboldmath$_{l}$} = $(\mu_{l_{1}},\ldots,
\mu_{l_{n}})^\top$ and \boldmath$\beta$\unboldmath$_{l}$ $=(\beta_{l_{0}},\ldots,\beta_{l_{p}})^\top$, $l = 1,2$. Here, $g_{1}($\mbox{\boldmath$\mu$\unboldmath$_{1}$}) and $g_{2}($\mbox{\boldmath$\mu$\unboldmath$_{2}$}) 
